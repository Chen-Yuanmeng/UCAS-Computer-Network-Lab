...
def ping_and_write_thread(net, ofile):
    t = time()
    h1, h2 = net.get('h1', 'h2')
...
def rtt_monitor(net, fname):
    ...
    ofile = open(fname, 'w')
    while 1:
        ping_thread = threading.Thread(target=ping_and_write_thread, args=(net, ofile,))
        ping_thread.start()
        sleep(0.1)
...
def set_qdisc_algo(net, algo):
    ...
    if algo.lower() not in algo_func_dict.keys():
        print('%s is not supported.' % (algo))
        sys.exit(1)

    r1 = net.get('r1')
    for func in algo_func_dict[algo.lower()]:
        r1.cmd(func)
