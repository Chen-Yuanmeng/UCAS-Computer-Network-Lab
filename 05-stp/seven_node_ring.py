...
if __name__ == '__main__':
    ...
    for i in names:
        ...
        node.cmd(f'./stp > {i}-output.txt 2>&1 &')
        # node.cmd(f'./stp-reference > {i}-output-reference.txt 2>&1 &')

    print("STP is running on seven nodes, start 30 seconds wait...")
    ...
    for i in names:
        ...
        # node.cmd('pkill -SIGTERM stp-reference')

    os.system('./dump_output.sh 7 > output-7.txt')
    # os.system('./dump_output-reference.sh 7 > output-reference-7.txt')
