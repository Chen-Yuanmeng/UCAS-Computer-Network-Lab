...
bool stp_port_enabled(stp_port_t *p)
{
	// in this lab, all ports are always enabled
	return stp_port_state(p) != ALTERNATE;
}
...
// 比较两个端口的优先级, 若 p1 优先级更高则返回 true
static bool stp_port_compare_priority(
		u64 p1_designated_root, u64 p2_designated_root,
		u32 p1_designated_cost, u32 p2_designated_cost,
		u64 p1_designated_switch, u64 p2_designated_switch,
		u16 p1_designated_port, u16 p2_designated_port)
{
	// 比较 root_id
	if (p1_designated_root != p2_designated_root) {
		return p1_designated_root < p2_designated_root;
	}

	// 比较 root_path_cost
	if (p1_designated_cost != p2_designated_cost) {
		return p1_designated_cost < p2_designated_cost;
	}

	// 比较 switch_id
	if (p1_designated_switch != p2_designated_switch) {
		return p1_designated_switch < p2_designated_switch;
	}

	// 比较 port_id
	if (p1_designated_port != p2_designated_port) {
		return p1_designated_port < p2_designated_port;
	}
		...
		struct stp_config *config)
{
	...
	bool recv_high = stp_port_compare_priority(
		config_root_id, p->designated_root,
		config_root_path_cost, p->designated_cost,
		config_switch_id, p->designated_switch,
		config_port_id, p->designated_port
	);

	if (recv_high) {
		// 收到的 config 优先级更高, 说明该网段应该通过对方端口连接根节点

		// 3. 将本端口的 config 替换为收到的 config
		...
		for (int i = 0; i < stp->nports; i++) {
			...
			if (!stp_port_is_designated(port)) {
				if (root_port /* != NULL */) {
					if (stp_port_compare_priority(
						port->designated_root, root_port->designated_root,
						port->designated_cost, root_port->designated_cost,
						port->designated_switch, root_port->designated_switch,
						port->designated_port, root_port->designated_port
					)) {
						root_port = port;
					}
				} else {
					// Root port is NULL, first undesignated port
					root_port = port;
				}
			}
		}
		...
		// 5. 更新剩余端口的 config
		// 对于所有端口，基于新的全局 stp 状态更新它们的 designated 字段
		for (int i = 0; i < stp->nports; i++) {
			stp_port_t *port = &stp->ports[i];
		...
		// 收到的 config 优先级更低
		if (stp_port_is_designated(p)) {
			// 本端口为指定端口, 需要发送 config 报文
			stp_port_send_config(p);
		}
	}
}

const char *stp_port_state_str(enum stp_port_state state)
{
	switch (state) {
		case ROOT:
			return "ROOT";
