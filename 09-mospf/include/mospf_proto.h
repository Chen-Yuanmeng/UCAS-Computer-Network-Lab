...
struct mospf_hdr {
	u8		version;	// MOSPF version (2)
	u8		type;		// MOSPF message type: 1 for Hello, 4 for LSU
	u16		len;		// Length of the MOSPF packet in bytes
	u32		rid;		// Router ID of the source router
	u32		aid;		// Area ID: Set to 0.0.0.0
	u16		checksum;	// Checksum of the MOSPF packet
...
struct mospf_lsu {
	u16		seq;		// LSU sequence number
	u8		ttl;		// Time to live
	u8		unused;		// Unused, set to zero
	u32		nadv;		// Number of advertisements
}__attribute__ ((packed));
