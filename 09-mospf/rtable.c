...
pthread_mutex_t rtable_mutex;

void init_rtable()
{
	init_list_head(&rtable);
	pthread_mutex_init(&rtable_mutex, NULL);
}
...
void try_add_rt_entry(u32 dest, u32 mask, u32 gw, iface_info_t *iface)
{
	...
	// Add new entry
	entry = new_rt_entry(dest, mask, gw, iface);
